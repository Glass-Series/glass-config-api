...
public interface GlassPacketListener {
    ...
    default void registerGlassPacket(String fullId, BiConsumer<GlassPacket, NetworkHandler> applicator, boolean clientBound, boolean serverBound) {
        net.glasslauncher.mods.networking.GlassNetworking.PACKET_HANDLERS.computeIfPresent(fullId, (id, applicator2) -> {throw new RuntimeException("Packet ID \"" + id + "\" already exists!");});

        net.glasslauncher.mods.networking.GlassNetworking.PACKET_HANDLERS.put(fullId, applicator);
        if (clientBound) {
            net.glasslauncher.mods.networking.GlassNetworking.CLIENT_BOUND_PACKETS.add(fullId);
        }
        if (serverBound) {
